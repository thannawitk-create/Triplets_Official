...
import { Play, Pause, SkipBack, SkipForward, Disc, Music, Repeat, Repeat1, Shuffle } from 'lucide-react';
...
export const FloatingMusicPlayer: React.FC<FloatingMusicPlayerProps> = ({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 px-2 sm:px-6 pb-3 pointer-events-none">
      <div className="max-w-4xl mx-auto bg-neutral-950/95 border border-neutral-800 rounded-2xl shadow-2xl backdrop-blur-xl pointer-events-auto overflow-hidden text-white transition-all">
        ...
        >
          <div
            className="h-full bg-gradient-to-r from-red-600 to-red-400 relative transition-all duration-75"
          ...
          <div className="flex items-center gap-3 min-w-0 cursor-pointer" onClick={onNavigateMusic}>
            ...
            <div className="min-w-0">
              <div className="font-bold text-xs sm:text-sm truncate flex items-center gap-1.5">
                <span>{activeSong.titleThai}</span>
                {activeSong.featuredArtist && (
                  <span className="text-[10px] text-red-400 font-mono bg-red-950 px-1.5 py-0.2 rounded border border-red-800">
                    {activeSong.featuredArtist}
                  </span>
                )}
              </div>
              <p className="text-[11px] text-neutral-400 font-mono truncate flex items-center gap-2">
                <span>TRIPLETS • {formatTime(currentTime)} / {formatTime(duration) || activeSong.duration}</span>
